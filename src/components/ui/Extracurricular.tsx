      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {extracurricular.map((item: { date: string, title: string, role: string, institution: string, description?: string, details?: string[] }, idx: number) => (
          <div key={idx} className="flex flex-col gap-4">
            ...
            <h4 className="text-xl font-medium tracking-tight uppercase leading-[1.3] text-[var(--foreground)]">
              {item.title}
            </h4>
            ...
            {item.details && item.details.length > 0 && (
              <ul className="flex flex-col gap-2 mt-2 border-t border-[var(--color-border)]/50 pt-4">
                {item.details.map((detail: string, dIdx: number) => (
                  <li key={dIdx} className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-[var(--color-muted)] mt-2 flex-shrink-0" />
